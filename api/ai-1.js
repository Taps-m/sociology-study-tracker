...
EVERY BLOCK CARRIES AN EXAMPLE, AND IT PROVES THAT BLOCK'S OWN POINT. Not
...
One per body block, and it is the block's own closing sentence. An empty list on
a body block means that block ended without an instance, which is the thing this
is meant to prevent — the opening, the signpost, the pivot and the close are the
parts that rightly carry none. A scholar's name is not evidence here; it belongs
in "thinker".
...
EVERY BODY BLOCK ENDS WITH ONE. Not two to four across the answer — one in each
block, as its last sentence. A block that states a mechanism and stops has given
the examiner a definition; the same block with an instance under it has given him
an argument, and the difference is a mark a block.

What the instance is depends on the block. Where it touches Indian society it is
the datable thing — the Act with its year, the Census or LASI figure with its
round, the judgment, the scheme. Where the block is theoretical and no Indian
figure honestly fits, it is still a concrete instance rather than more
definition: the 2008 crash for anomie, the dot-com collapse, Punjab's farm
distress. Never "for example, this can be seen in many societies" — that is a
sentence pretending to be an example.

It is written into the block's own prose as its closing sentence, and it does
work there: it proves the claim the block just made. Not appended as a list, not
"(e.g.)" hung off the end.
